...
class Apollyon(sc2.BotAI):
    def __init__(self, use_model=False):
        ...
        if self.use_model:
            print("using model")
            self.model = keras.models.load_model("Apollyon_Cruiser")
    ...
    async def on_step(self, iteration):
        ...
        await self.upgrade_starport()
        await self.build_fusioncore()
...
####################################################################### GAME METHODS ##########################################################################
    ...
    async def build_supplydepot(self):
        ...
        if self.supply_left < 14:
            await self.build(SUPPLYDEPOT, near=centers.position.towards(self.game_info.map_center, 10))   
...
## MILITARY BUILDINGS
    ...
    async def build_barracks(self):
        centers = self.units(COMMANDCENTER).first
        if self.units(SUPPLYDEPOTLOWERED).exists and self.units(COMMANDCENTER).amount == 3:
                if self.can_afford(BARRACKS) and self.units(BARRACKS).amount < 2:
                    if not self.already_pending(BARRACKS) and self.units(BARRACKSTECHLAB).amount < 2:
            ...
            if self.units(FACTORY).amount < 1 and not self.already_pending(FACTORY):
                if self.units(MARINE).amount > 5:
                    if self.can_afford(FACTORY):    
                        await self.build(FACTORY, near=centers.position.towards(self.game_info.map_center, 6))


    async def build_starport(self):
        centers = self.units(COMMANDCENTER).first 
        if self.units(FACTORY).exists:
            if self.units(STARPORTTECHLAB).amount < 3 and not self.already_pending(STARPORT):
                if self.units(MARINE).amount > 5:
                    if self.can_afford(STARPORT):    
                        await self.build(STARPORT, near=centers.position.towards(self.game_info.map_center, 14)) 


    async def upgrade_starport(self):
        for starport in self.units(STARPORT).ready:
            if self.can_afford(STARPORTTECHLAB):
                if starport.add_on_tag == 0:
                    await self.do(starport.build(STARPORTTECHLAB))
 
                           


    async def build_fusioncore(self):
        centers = self.units(COMMANDCENTER).first 
        if self.units(STARPORT).exists: 
            if self.units(FUSIONCORE).amount < 1:
                if self.can_afford(FUSIONCORE) and not self.already_pending(FUSIONCORE):    
                    await self.build(FUSIONCORE, near=centers.position.towards(self.game_info.map_center, 6)) 
                            
       

## MILITARY UNITS  

## COMPOSITION ~ 10 Marines, 10 Marauders for defense; Max BattleCruisers  
                    

    async def build_army(self):
        for barrack in self.units(BARRACKS).ready.noqueue:
            if self.can_afford(MARINE) and self.supply_left > 0:
                if self.units(MARINE).amount < 10 and self.can_afford(MARINE):
        ...
        for starport in self.units(STARPORT).ready.noqueue:
            if self.units(FUSIONCORE).exists:
                if self.can_afford(BATTLECRUISER) and self.supply_left > 6:
                        await self.do(starport.train(BATTLECRUISER))    
...
## ACTIONS
    ...
    async def defend(self):
        army = {MARINE: [24, 5],
                MARAUDER: [12, 1],
                BATTLECRUISER: [5, 0],}           ## group units (attack/def)
    ...
    async def attack(self):
        ...
        if len(self.units(BATTLECRUISER).idle) > 3:
            ...
            if self.use_model:
                ...
                choice_dict ={0: "Wait",
                              1: "Eliminate Threat",
                              2: "Attack Enemy Object",
                              3: "Attack Enemy Base"}

                print("I'm going to {}".format(choice_dict[choice]))              
            ...
            if self.iteration > self.choose_next_action:
                ...
                if target:
                    for vr in self.units(BATTLECRUISER).idle:
