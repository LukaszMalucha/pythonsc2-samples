...
model.add(Dense(4, activation='softmax'))   ## actions
...
## Iterate over lists to check the data length
...
for i in range(hm_epochs):
	...
	increment = 200
	not_maximum = True
	all_files = os.lisstdir(train_data_dir)
	maximum = len(all_files)
	random.shuffle(all_files)

	while not_maximum:
		print("{}:{}".format(current, current+increment))
		no_attacks = []
        ...
        ## Sliec from the beginning to the lowest_data length
